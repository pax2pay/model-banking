		...
		for (const iin of Iin.values)
			if (iin !== "totalIdx") {
				...
				const iinCardsWithinRange = iinCards.filter(card => {
					const created = isoly.DateTime.is(card.created) ? isoly.DateTime.getDate(card.created) : card.created
				...
				const activeCards = iinCardsWithinRange.filter(
					card =>
						card.history.some(history => {
							const created = isoly.DateTime.is(history.created)
								? isoly.DateTime.getDate(history.created)
								: history.created
							return history.status == "cancelled" && created > range.end
