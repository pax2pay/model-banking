...
describe("Analytics transactions mapping", () => {
	it("Configuration is checks", () => {
		expect(pax2pay.Analytics.Configuration.Mapping.type.is(pax2pay.Analytics.Configuration.Transaction.mapping)).toBe(
			true
		)
		expect(pax2pay.Analytics.Configuration.Schema.type.is(pax2pay.Analytics.Configuration.Transaction.schema)).toBe(
			true
		)
	})
	it("Mapping", () => {
