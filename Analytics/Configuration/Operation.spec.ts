...
describe("Analytics operations mapping", () => {
	it("Configuration is checks", () => {
		expect(filter.Mapping.RecordWithSelector.type.is(pax2pay.Analytics.Configuration.Operation.mapping)).toBe(true)
		expect(listener.BigQueryApi.BaseField.type.array().is(pax2pay.Analytics.Configuration.Operation.schema)).toBe(true)
	})
	it("Mapping", () => {
