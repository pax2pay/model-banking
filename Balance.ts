	...
	export function add(addendee: Extended, addend: Extended, currency: isoly.Currency): Extended {
		...
		return update(currency, {
			...
			reserved: {
				...
				incoming: isoly.Currency.add(
					currency,
					updated[0]!.reserved?.incoming ?? 0,
					updated[1]!.reserved?.incoming ?? 0
				),
				outgoing: isoly.Currency.add(
					currency,
					updated[0]!.reserved?.outgoing ?? 0,
					updated[1]!.reserved?.outgoing ?? 0
				),
			},
		})
	}
