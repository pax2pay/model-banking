...
describe("authenticator", () => {
	...
	const thingy = userwidgets.User.Key.Issuer.create("jest", "all ages", publicKey, privateKey)
	const authenticator = new pax2pay.Authenticator(publicKey)
	// it("role example", async () => {
	// 	const realm: pax2pay.Realm | undefined = "test"
	// 	const authenticated = await authenticator.authenticate("aaaa", {
